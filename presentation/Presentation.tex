...
\begin{block}{Problem}
\begin{itemize}
    \item Gegeben zwei Strings $r, s \in \Sigma^*$.
    \item Gesucht: Die Länge des längsten gemeinsamen Teilwort $lcs(r, s)$ von $r$ und $s$
\end{itemize}
\end{block}
\begin{example}
$lcs(abab, aab) = 2$
...
% \begin{frame}
% \frametitle{Tiefster gemeinsamer Vorfahre}
% \begin{figure}
% \begin{tikzpicture}[
%     font = \small,
%     level 1/.style = {level distance = 7mm, sibling distance = 35mm},
%     level 2/.style = {level distance = 7mm, sibling distance = 25mm},
%     level 3/.style = {level distance = 7mm, sibling distance = 15mm},
%     edge from parent/.style = {->, draw}
%     ]
%     \node[ghostbranch]{}
%     child[child anchor = north] {
...
%         }
%         child {
%             node[branch, green] (d2) {}
%             edge from parent
%         }
%         edge from parent
%     };
%     \node[subtree] at (d1.center) {};
%     \node[subtree] at (d2.center) {};
% \end{tikzpicture}
% \end{figure}
% \begin{definition}[Lowest common ancestor]
% \begin{itemize}
%     \item $\overline{w}, \overline{v}$ zwei Knoten aus einem GST $GST_D$
%     \item $lca(\overline{w}, \overline{v}) = \overline{t}$ genau dann wenn $\overline{t}$ der tiefste Knoten im Baum ist, der sowohl Vorfahre von $\overline{w}$ und von $\overline{v}$ ist.
% \end{itemize}
% \end{definition}
% \end{frame}
...
\begin{tikzpicture}[
    ...
    \node[yshift = 3mm, xshift = -7mm] at (w1.center) {$\overline{w}$};
    \node[yshift = 3mm, xshift = -7mm] at (w2.center) {$\overline{w}$};
\end{tikzpicture}
\end{figure}
\end{example}
