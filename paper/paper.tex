...
Bereits 1990 wurden Suffix Arrays \cite{Manber1990} von \textit{U. Manber} und \textit{Gene Myers} als speicherplatzeffiziente Alternative zu Suffix Trees vorgestellt. Seitdem bei den Suffix Trees bezüglich des Speichplatzbedarfs keine nennenswerten Fortschritte mehr erzielt wurden, verschiebt sich der Schwerpunkt der Forschung immmer mehr in Richtung Suffix Arrays. Anfangs existierten wie bei den Suffix Trees keine effizienten Konstruktionalgorithmen. 2003 wurden gleich drei Algorithmen veröffentlicht, die das Konstruktionsproblem in linearer Zeit lösen, darunter auch \cite{Karkkainen2003}.

Auf den folgenden Seiten betrachten wir Suffix Trees und Suffix Arrays aus der Perspektive der funktionalen Programmierung. Zunächst beschäftigen wir uns mit den Suffix Trees. Nachdem wir in Abschnitt \ref{sec:Terminologie} die notwendigen Grundlagen und die verschieden Suffix Tree Typen besprochen haben, gehen wir im nächsten Abschnitt \ref{sec:Anwendungsgebiete} auf die Anwendungsgebiete von Suffix Trees näher ein und schließen das Kapitel \ref{chap:SuffixTree} mit einer Erläuterung des funktionalen Suffix Tree Konstruktionsalgorithmus aus \cite{Giegerich1995} ab.

In Kapitel \ref{sec:SuffixArray} befassen wir uns mit den Suffix Arrays. Zuerst diskutieren wir in Abschnitt \ref{sec:Motivation} die Gründe für den Fokuswechsel von Suffix Trees zu Suffix Arrays und stellen anschließend den in \cite{Karkkainen2003} publizierten Suffix Array Konstruktionsalgorithmus vor.
\end{abstract}
